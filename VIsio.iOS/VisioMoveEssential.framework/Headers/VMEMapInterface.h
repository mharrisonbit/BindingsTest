...
/**
 * Set's the view mode to either VMEViewModeFloor or VMEViewModeGlobal (depending
 * on where the position is located).  Updates the camera's altitude so that
 * it is within the min/max range supplied.  If the camera is already within that 
 * range, then it won't be moved.
 *
 * @param position The position to focus the camera on.
 * @param heading The heading to be applied to the camera.
 * @param minAltitude The minimum altitude the camera can have.
 * @param maxAltitude The maximum altitude the camera can have.
 *
 * @version 1.3
 */
+(instancetype) cameraUpdateForPosition:(VMEPosition*)position
                                 heading:(VMECameraHeading*)heading
